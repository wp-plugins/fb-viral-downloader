<?php
class FB_Viral_Downloader_Settings {
  ...
  public function __construct() {
    ...
    // Settings tabs
    add_action('settings_page_fb_viral_downloader_general_tab_init', array(&$this, 'general_tab_init'), 10, 1);
    add_action('settings_page_fb_viral_downloader_logs_tab_init', array(&$this, 'logs_tab_init'), 11, 1);
  }
  ...
  public function add_settings_page() {
    ...
    add_menu_page(
        __('Viral Downloader Settings', $FB_Viral_Downloader->text_domain), 
        __('Viral Downloader Settings', $FB_Viral_Downloader->text_domain), 
        'manage_options', 
        'fb-viral-downloader-setting-admin', 
        array( $this, 'create_fb_viral_downloader_settings' ),
        $FB_Viral_Downloader->plugin_url . 'assets/images/dualcube.png'
    );
  ...
  function get_dc_settings_tabs() {
    global $FB_Viral_Downloader;
    $tabs = apply_filters('fb_viral_downloader_tabs', array(
      'fb_viral_downloader_general' => __('General Settings', $FB_Viral_Downloader->text_domain),
      'fb_viral_downloader_logs' => __('Download Logs' , $FB_Viral_Downloader->text_domain)
    ));
    return $tabs;
  }
  
  function dc_settings_tabs( $current = 'fb_viral_downloader_general' ) {
    if ( isset ( $_GET['tab'] ) ) :
      $current = $_GET['tab'];
    else:
      $current = 'fb_viral_downloader_general';
    endif;
    
    $links = array();
    foreach( $this->tabs as $tab => $name ) :
      if ( $tab == $current ) :
        $links[] = "<a class='nav-tab nav-tab-active' href='?page=fb-viral-downloader-setting-admin&tab=$tab'>$name</a>";
      else :
        $links[] = "<a class='nav-tab' href='?page=fb-viral-downloader-setting-admin&tab=$tab'>$name</a>";
      endif;
    endforeach;
   ...
   */
  public function create_fb_viral_downloader_settings() {
    global $FB_Viral_Downloader;
    ?>
    <div class="wrap">
      <?php $this->dc_settings_tabs(); ?>
      <?php
      $tab = ( isset( $_GET['tab'] ) ? $_GET['tab'] : 'fb_viral_downloader_general' );
    ...
    <?php
    do_action('fb_viral_downloader_dualcube_admin_footer');
  }
      ...
      foreach($tab_options['sections'] as $sectionID => $section) {
        ...
        if(isset($section['fields'])) {
          foreach($section['fields'] as $fieldID => $field) {
            if(isset($field['type'])) {
              $field = $FB_Viral_Downloader->dc_wp_fields->check_field_id_name($fieldID, $field);
  ...
  function general_tab_init($tab) {
    global $FB_Viral_Downloader;
    $FB_Viral_Downloader->admin->load_class("settings-{$tab}", $FB_Viral_Downloader->plugin_path, $FB_Viral_Downloader->token);
    new FB_Viral_Downloader_Settings_General($tab);
  }
  
  function logs_tab_init($tab) {
    global $FB_Viral_Downloader;
    $FB_Viral_Downloader->admin->load_class("settings-{$tab}", $FB_Viral_Downloader->plugin_path, $FB_Viral_Downloader->token);
    new FB_Viral_Downloader_Settings_Logs($tab);
  }
  ...
  public function hidden_field_callback($field) {
    ...
    $FB_Viral_Downloader->dc_wp_fields->hidden_input($field);
  }
  ...
  public function text_field_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? esc_attr( $this->options[$field['name']] ) : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->text_input($field);
  }
  ...
  public function textarea_field_callback($field) {
    ...
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->textarea_input($field);
  }
  ...
  public function wpeditor_field_callback($field) {
    ...
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->wpeditor_input($field);
  }
  ...
  public function checkbox_field_callback($field) {
    ...
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->checkbox_input($field);
  }
  ...
  public function radio_field_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? esc_attr( $this->options[$field['name']] ) : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->radio_input($field);
  }
  ...
  public function select_field_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? esc_textarea( $this->options[$field['name']] ) : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->select_input($field);
  }
  ...
  public function upload_field_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? esc_attr( $this->options[$field['name']] ) : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->upload_input($field);
  }
  ...
  public function multiinput_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? $this->options[$field['name']] : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->multi_input($field);
  }
  ...
  public function colorpicker_field_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? esc_attr( $this->options[$field['name']] ) : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->colorpicker_input($field);
  }
  ...
  public function datepicker_field_callback($field) {
    ...
    $field['value'] = isset( $this->options[$field['name']] ) ? esc_attr( $this->options[$field['name']] ) : $field['value'];
    $field['name'] = "dc_{$field['tab']}_settings_name[{$field['name']}]";
    $FB_Viral_Downloader->dc_wp_fields->datepicker_input($field);
  }
